# -*- coding: utf-8 -*-
"""
Created on Tue Dec 14 23:55:40 2021
...
class Window_Start(object):
    ...
    def __init__(self, master):
        ...
        self.user_s = Label(self.top, text="Enter info", font=('Bookman',10)).grid(row=5,column=2)
        self.user_label = Label(self.top, text=" Username: ", font=('Bookman',10)).grid(row=6,column=1)
        self.pass_label = Label(self.top, text=" Password: ", font=('Bookman',10)).grid(row=7,column=1)
        self.user_entry = Entry(self.top, width=30)
        self.user_entry.grid(row=6, column=2)
        self.pass_entry = Entry(self.top, show="*",width=30)
        self.pass_entry.grid(row=7, column=2, padx=5)
        self.sub_button = Button(self.top, text='Submit',command=self.entry_access, font=('Bookman',10))
        self.sub_button.grid(columnspan=1, row=8, column=2)
...
class Account:
    ...
    def display(self):
        ...
        self.delete_button = Button(self.window, text='Delete', fg='red', command=self.delete_account)
...
def create_account(window, title_val, username_val, password_val, is_new_account):
    if title_val !='' and username_val!='' and password_val !='' :
        global total_count
        ...
        messagebox.showerror("Information Missing", "Complete all the required information.")
...
def main():
    ...
    title = Entry(window, font=('Bookman',12), width = 25)
    username = Entry(window, font=('Bookman',12), width = 25)
    password = Entry(window, show='*',font=('Bookman',12), width = 25)
    submit = Button(window, text=" Add ", 
    command = lambda: submit_entry(window, title, username, password, True), font=('Bookman',10))
    
    #query_btn = Button(window, text = "Show Records", command = lambda:view_entry(window, title, username, password, True),font=('Bookman',10))
    #query_btn.grid(row=6, column=2,sticky=E)
    ...
    submit.grid(column=1, row=6,sticky=E)
